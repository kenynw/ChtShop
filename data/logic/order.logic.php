...
class orderLogic {
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '', $if_update_account = true, $if_quque = true) {
        try {
            ...
            //add by lai 释放优惠券<<
               $orderCommonModel = Model('order_common');
               $voucherInfo=$orderCommonModel->field("voucher_code")->where("order_id='$order_id'")->find();
              if(isset($voucherInfo['voucher_code'])){
                  $voucherModel=Model("voucher");
                  $voucherCondition = array();
                  $voucherCondition['voucher_code'] = $voucherInfo['voucher_code'];
                  $voucherCondition['voucher_owner_id'] = $order_info['buyer_id'];
                  $voucherCondition['voucher_state'] = 2;
                  $voucherState = $voucherModel->table("voucher")->where($voucherCondition)->find();
                  if ($voucherState){
                      $updateData = array();
                      $updateData['voucher_state'] = 1;
                      $voucherModel->table("voucher")->where($voucherCondition)->update($updateData);
                  }
              }
               //$voucherCondition['voucher_store_id']=$order_info['buyer_id'];
            //add by lai 释放优惠券>>
