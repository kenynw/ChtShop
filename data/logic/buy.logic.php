...
class buyLogic {
    ...
    private function _createOrderStep4() {
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            //add by lai 用于免费茶样 如果商品都属于免费茶样，而且无需支付运费 则更好 订单状态 为已经付款暂时在同个店铺里面 <<
            if((($should_goods_total+$order['shipping_fee'])==0)&&(count($store_cart_list)==1)){
                $orderData=array();
                $orderData['order_state']=20;
                $orderData['payment_code']='sample';
                $model_order->table("order")->where("order_id='$order_id'")->update($orderData);
                $orderPayData=array();
                $orderPayData['api_pay_state']=1;
                $model_order->table("order_pay")->where("pay_id='$order_pay_id'")->update($orderPayData);

                $orderLogData = array();
                $orderLogData['order_id'] = $order_id;
                $orderLogData['log_role'] = "买家";
                $orderLogData['log_user'] = $member_name;
                $orderLogData['log_msg'] = '免运费样品 无需收款';
                $orderLogData['log_orderstate'] = ORDER_STATE_PAY;
                $model_order->addOrderLog($orderLogData);
            }
            //end >>
