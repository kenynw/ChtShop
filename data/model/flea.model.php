...
class fleaModel extends Model {
	 ...
	 * @param	int $goods_id 商品id
	 */ 
	public function dropGoods($goods_id, $member_id) {
		if(empty($goods_id)) {
			return false;
		}
		...
		if($del_state) {
			...
			if(is_array($image_more) && !empty($image_more)){
				foreach ($image_more as $v){
					@unlink(UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.empty($_SESSION['member_id']) ? $member_id : $_SESSION['member_id'].DS.$v['file_name']);
				}
			}
