...
class goods_browseModel extends Model
{
     ...
     * @return array
     */
    public function getGuessLikeGoods($member_id = 0, $shownum = 0, $field = '*')
    {
        $shownum = ($t = intval($shownum)) > 0 ? $t : 0;
...
//        $goodslist = Model('goods')->getGoodsOnlineList(array('goods_id'=>array('in',$goodsid_arr)));
        $goods_list = $model_goods -> getGoodsOnlineList($where, $field, 0, 'rand()', $shownum);

        if (empty($goods_list)) {
            $goods_list = $model_goods -> getGoodsOnlineList(array(), $field, 0, 'rand()', $shownum);
        }
        return $goods_list;
    }
