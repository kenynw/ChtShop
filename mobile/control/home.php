...
class homeControl extends mobileHomeControl{
    ...
    public function home_page_apiOp(){
        ...
        //$member_lable="cate_id=3_258&a_id=3059_3512&brand_id=444";
        $data=array();
        $data['isLogin']=isset($logState)?$logState:0;
        if($page==1) {
            //首页轮播图
            ...
            $flashSalePic = $carouselModel->where("location='3'")->find();
            if ($carouselFigure) {
            ...
            $data['carouselFigure'] = isset($carouselFigureCheck) ? $carouselFigureCheck : '';
            //免费茶样
            /*
...
//            $goodsSample['sample_image']=$tempArray;
//            $data['goodsSample'] = isset($goodsSample)? $goodsSample:'';
            if(isset($tempArray)) $data['goodsSample'] = $tempArray;
                //限时抢购
            $model_xianshi_goods = Model('p_xianshi_goods');
            $xianshi_item = $model_xianshi_goods->getXianshiGoodsCommendList(1);
            //list($host,$imageName,$imageType)=explode('.',$flashSalePic);
            $imageType=substr($flashSalePic['image'],-3);
            $imageName=substr($flashSalePic['image'],0,-4);
            $xianshi_item[0]['goods_image']=array();
            $xianshi_item[0]['goods_image']['original_pic']=$flashSalePic['image'];
            $xianshi_item[0]['goods_image']['bmiddle_pic']=$imageName.'_360.'.$imageType;
            $xianshi_item[0]['goods_image']['small_pic']=$imageName.'_60.'.$imageType;
            $flashSaleArray=array();
            $flashSaleArray['goods_image']=$xianshi_item[0]['goods_image'];
            $flashSaleArray['end_time']=$xianshi_item[0]['end_time'];
            $data['flashSale'] = isset($flashSaleArray) ? $flashSaleArray : '';
            //品茶师推荐
            $tastersRecommendSql="select g.goods_id,g.goods_name,g.goods_image,g.goods_price,g.goods_promotion_price,g.goods_marketprice,g.goods_promotion_type,g.store_id,r.recommend_score,r.recommend_taste,r.recommend_light,r.recommend_aroma,r.recommend_leaf from shopnc_tasters_recommend as r left join shopnc_goods as g on r.recommend_goods_id=g.goods_id where g.goods_verify='1' and g.goods_commend='1' and r.recommend_state='1' order by r.recommend_sort desc limit 4";
            $goodsModel = Model("");
            $tastersRecommends = $goodsModel->query($tastersRecommendSql);
            $tastersTem=array();
        ...
        if(!$logState){
            $member_lable="cate_id=3_258&a_id=3059_3512&brand_id=444";
            $guessYouLike=$this->get_likeOp($member_lable,$page);
            //$guessYouLike=$goodsModel->field("goods_id,goods_name,goods_image,goods_price,goods_promotion_price,goods_marketprice,goods_promotion_type,store_id")->where("goods_state='1'")->order("goods_click desc")->page(6)->select();
        }else{
        ...
        //情形2-1 登陆且有浏览过页面
            ...
            if($browseRecord[0]) {
                ...
                //echo $gcid;
                $guessYouLike=$goodsModel->where($gcid." and goods_state='1'")->limit(6)->select();
            }else{
        //情形2-2 登陆没有浏览页面
                $guessYouLike=$goodsModel->where("goods_state='1'")->order("goods_click desc")->page(6)->select();
            }
         */
          //有标签
       ...
       // print_r($guessYouLike);
      $temGuess=array();
      if($guessYouLike){

          $goodsCommonModel=Model("goods_common");
          foreach($guessYouLike as $value){
              list($imageName,$imageType)=explode(".",$value['goods_image']);
              $value['goods_image']=array();
              $value['goods_image']['original_pic']=Goods_Img_Patch.$value["store_id"].'/'.$imageName.'.'.$imageType;
              $value['goods_image']['bmiddle_pic']=Goods_Img_Patch.$value["store_id"].'/'.$imageName.'_360.'.$imageType;
      ...
      $data['guessYouLike']=isset($temGuess)?$temGuess:'';
        if($page==1){
            //add ceshi
