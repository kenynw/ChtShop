...
class goods_sampleControl extends mobileHomeControl{
    ...
    public function indexOp(){
        ...
        if($page<1){
         output_json(0,$data,'page  参数有误必须是大于0的整数');
        }
        $condition=array();
        ...
        if($page==1){
            ...
            if(!$goodsSample){
                output_json(0,$data,'样品没今日数据');
                die();
            }
            $imageArray=explode(",",$goodsSample['sample_image']);
            $goodsSample['sample_image']=array();
            foreach($imageArray as $key=>$v){
                $value=IMAGE_PACH.$v;
                $goodsSample['sample_image'][$key]['original_pic']=$value;
                list($str1,$str2)=explode('.',$v);
                $midpic=IMAGE_PACH.$str1."_360".'.'.$str2;
                $goodsSample['sample_image'][$key]['bmiddle_pic']=$midpic;
            }
            /*检测是否有权限*/
            if($_POST['key']){
                $mbUserModel=Model("mb_user_token");
                $memberInfo=$mbUserModel->where("token='".$_POST['key']."'")->find();
                if(!$memberInfo){
                    $goodsSample['allow']=0;
                    $goodsSample['msg']='key不正确';
                }else {
                    $buyer_id=$memberInfo['member_id'];
                    $goods_id = $goodsSample['sample_link'];
                    $sql = "select gs.goods_id,o.add_time from shopnc_order_goods as gs left join shopnc_order as o on gs.order_id=o.order_id where gs.goods_id=$goods_id and o.add_time>" . $goodsSample["sample_start_time"] . " and o.add_time<" . $goodsSample["sample_end_time"] . " and o.buyer_id='" .$buyer_id. "'";
                    $model = Model();
                    $result = $model->query($sql);
                    if($result){
                        $goodsSample['allow']=0;
                        $goodsSample['msg']='已经申领过了';
                    }else{
                        $goodsSample['allow']=1;
                        $goodsSample['msg']='可以申领';
                    }
                }
            }
            /**/
            ...
            //$overdueGoodsSample=$sampleModel->where($condition)->order("sample_end_time desc")->page(2)->select();
            //$data['overdueGoodsSample']=!empty($overdueGoodsSample)?$overdueGoodsSample:'';
            //$data['image_path']=IMAGE_PACH;
