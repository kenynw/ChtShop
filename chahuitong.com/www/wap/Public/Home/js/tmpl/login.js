$(function(){
    ...
    +'</div>';
	//茶市
	var chashiid=GetQueryString("chashi");
	
	//alert(chashiid);
	
	if(chashiid){
		
		$("input[name='chashi']").val(chashiid);
		...
		$.ajax({
			...
			success:function(result){
				if(result){
					delCookie('username');
					delCookie('key');
					location.href = WapSiteUrl+'/index.php/Home/index/login';
				}
			}
		});
	});	
	...
	if($("input[name='chashi']").val()!=0){
		referurl="http://www.chahuitong.com/mobile/app/b2b/index.php/Home/index/index";
		}
	...
	$('#loginbtn').click(function(){//会员登陆
		...
		if($.sValid()){
	          $.ajax({
				...
				data:{usermobile:username,password:pwd,client:client},
				dataType:'json',
				success:function(result){
					 //alert(result.datas)
