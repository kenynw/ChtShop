...
<input type="search" name="search" placeholder="输入号数/姓名">
</form>
<button id="search"><img src="http://www.chahuitong.com/wap/Public/Home/img/searchgreen.png">搜索</button>
<a href="/wap/index.php/Home/Index/activity_vote">投票</a>
</nav>
<div id="success">
<div id="msses"></div>
</div>
                ...
                success: function (data) {
                    if(data.code==1) {
                        for (var i = 0; i < data.data.length; i++) {
                            var html = '<li id="' + data.data[i].enroll_member_id + '" class="choose" ><img class="img" src="' + data.data[i].enroll_member_avatar + '">';
                            html += '<p class="piaoshu" style="display: block;">'+ data.data[i].enroll_id+'号'+ data.data[i].enroll_member_name+'<span><mark>'+data.data[i].enroll_vote_number+'</mark>票</span></p></li>';
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.code==1) {
                            $("#toupiao ul").html('');
                            for (var i = 0; i < data.data.length; i++) {
                                var html = '<li id="' + data.data[i].enroll_member_id + '" class="choose" ><img class="img" src="' + data.data[i].enroll_member_avatar + '"><p class="xingming" style="display:none" >';
                                html += data.data[i].enroll_member_name + '</p><p class="piaoshu" style="display:block">' + data.data[i].enroll_id+'号'+data.data[i].enroll_member_name + '<span><mark>';
                                html += data.data[i].enroll_vote_number + '</mark>票</span></p><div class="checkbox1" style="display:none" did="' + data.data[i].enroll_member_id + '"><input id="a' + data.data[i].enroll_member_id + '" type="checkbox" name="vote"><label for="a' + data.data[i].enroll_member_id + '" ><span></span></label></div></li>';
                                $("#toupiao ul").append(html);
                            }
                            //page
                            page=100;
