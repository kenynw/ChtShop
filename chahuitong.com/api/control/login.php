...
class loginControl extends mobileHomeControl {
	...
	public function indexOp(){
        ...
        if(  empty($_POST['password'])  || !in_array($_POST['client'], $this->client_type_array)) {
            output_error('登录失败');
              }
		 }
        ...
        if(!empty($member_info)) {
            ...
            if($token) {
                if($_POST['version']){
                    output_json(1,array('key'=>$token),'登录成功');
                    die();
                }
                output_data(array('username' => $member_info['member_name'],'key' => $token,'avatar'=>$member_info['member_avatar']) );
            } else {
                if($_POST['version']){
