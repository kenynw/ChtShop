...
class member_sns_informControl extends mobileSNSControl {
    ...
    public function inform_traceOp() {
        ...
        if ($trace_id < 0) output_json(0, false, Language::get("wrong_argument"));
        ...
        if (!empty($inform_info)) output_json(0, false, '已经举报过了');

        $model_trace = Model('sns_tracelog');
        $trace_info = $model_trace->getTraceByID($trace_id, 'trace_title');
        if (empty($trace_info)) output_json(0, false, '举报的动态不存在');
        ...
        if (!$result) output_json(0, false, '请求超时');
        output_json(1, true);
    }
    ...
    public function inform_homeOp() {
        ...
        if ($mid < 0) output_json(0, false, Language::get("wrong_argument"));

        if ($mid == $this->member_info['member_id']) output_json(0, false, '不能举报自己');
        ...
        if (empty($member_info)) output_json(0, false, '举报的动态不存在');
