...
class member_cartControl extends mobileMemberControl {
    ...
    public function cart_addOp() {
        ...
        if($goods_info['is_goods_sample']){
            ...
            $goodsSampleInfo=$goodsSampleModel->where("sample_link='".$goods_info['goods_id']."'")->find();
    ...
    public function cart_delOp() {
        ...
        if($cart_id > 0) {
			...
			//add by lai			
			$cart_cache=Model();			
			$sql="insert into shopnc_cart_cache select * from shopnc_cart where cart_id='$cart_id' and buyer_id='{$condition['buyer_id']}'";			
			@$cart_cache->query($sql);	
			//echo $result;			 			
			//end
            $delResult=$model_cart->delCart('db', $condition);
        }
        if($delResult){
            if($_POST['version']){
                $cat_array=array('cart_id'=>$cart_id);
                output_json(1,$cat_array,"删除完成");
                die();
            }else{
                output_data('1');
            }
	 ...
	 public function cart_againOp() {
       $cart_id = intval($_POST['cart_id']);
        if($cart_id > 0) {
			...
			//add by lai			
			$cart_cache=Model();			
			$sql="insert into shopnc_cart select * from shopnc_cart_cache where cart_id='$cart_id' and buyer_id='{$condition['buyer_id']}'";	
			$sql1="delete from shopnc_cart_cache where cart_id='$cart_id' and buyer_id='{$condition['buyer_id']}'";		
			@$cart_cache->query($sql);	
