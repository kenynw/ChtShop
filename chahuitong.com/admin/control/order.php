	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
			...
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				$data = $model_order->getOrderList($condition,'','*','order_state desc,order_id desc',self::EXPORT_SIZE,array('order_goods','member'));
				$this->createExcel($data);
			}
		}else{	//下载
			$limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
			$limit2 = self::EXPORT_SIZE;
			$data = $model_order->getOrderList($condition,'','*','order_state desc,order_id desc',"{$limit1},{$limit2}",array('order_goods'));
			$this->createExcel($data);
		}
	}
	...
	private function createExcel($data = array()){
		...
		foreach ((array)$data as $k=>$v){
			...
			if(isset($v['extend_order_goods'])){
				foreach($v['extend_order_goods'] as $i=>$og){
					$goodsNameStr .= $og['goods_name'].'(￥'.$og['goods_price'].'),';
					$brandNameStr[] = isset($og['brand_name']) && $og['brand_name'] ? $og['brand_name'] : '';
				}
			}
			$memberTrueName = '';
			if(isset($v['extend_member'])){
				$memberTrueName = $v['extend_member']['member_truename'];
			}
			$brandNameStr = array_unique(array_filter($brandNameStr));
			$tmp[] = array('data'=>implode(',',$brandNameStr));
			$tmp[] = array('data'=>trim($goodsNameStr,','));
			$tmp[] = array('data'=>$memberTrueName);
			$excel_data[] = $tmp;
		}
