...
class goods_classControl extends SystemControl{
	...
	/*分类广告*/
	public  function goods_class_advOp(){
		$gc_id=intval($_GET['gc_id']);
		$advModel=Model("advertisement");
		//$sql="select * from shopnc_advertisement where class_id='$gc_id'";
		$advs=$advModel->where("class_id='$gc_id'")->select();
		Tpl::output('advs',$advs);
        Tpl::output('class_id',$gc_id);
		Tpl::showpage('advs.index');
		}
   /*添加分类广告*/	
   public function goods_add_advOp(){
	    Tpl::showpage('addAdvs.index');   
	   }	
   /*保存广告*/
   public function adv_saveOp(){
		...
		$array['goods_link']=$_POST['goods_link'];
		$array['`order`']=$_POST['order'];
		$result=$advModel->insert($array);
		if($result){
			showMessage('添加成功');
			}else{				
			showMessage('添加失败');	
	...
	/*删除对应广告*/  
	public function goods_del_advOp(){
		$id=intval($_GET['id']);
		$advModel=Model("advertisement");
		$result=$advModel->where("id='$id'")->delete();
		if($result){
			showMessage('删除成功');
			}else{				
			showMessage('删除失败');	
